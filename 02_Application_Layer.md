...
- [`AudioTrack`](frameworks/base/media/java/android/media/AudioTrack.java) — 继承PlayerBase，实现AudioRouting + VolumeAutomation
...
**状态常量详解**（定义于 [`AudioTrack.java`](frameworks/base/media/java/android/media/AudioTrack.java)）：
...
AudioTrack使用Builder模式构造，Builder.build()内部逻辑（[`AudioTrack.java`](frameworks/base/media/java/android/media/AudioTrack.java)）：
...
**源码实现**（[`AudioTrack.java`](frameworks/base/media/java/android/media/AudioTrack.java)）：
...
**shouldEnablePowerSaving()决策逻辑**（[`AudioTrack.java`](frameworks/base/media/java/android/media/AudioTrack.java)）：
...
Builder中对Offload模式有严格的兼容性检查（[`AudioTrack.java`](frameworks/base/media/java/android/media/AudioTrack.java)）：
...
构造函数最终调用native_setup（[`AudioTrack.java`](frameworks/base/media/java/android/media/AudioTrack.java)）：
...
#### AudioTrack::set()（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）
...
#### createTrack_l()深度分析（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）
...
AudioTrack的write()方法是应用写入音频数据的核心入口（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）：
...
**关键代码**（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）：
...
这是AudioTrack零拷贝机制的核心数据结构（[`AudioTrackShared.h`](frameworks/av/include/private/media/AudioTrackShared.h)）：
...
**Streaming模式核心字段**（[`AudioTrackShared.h`](frameworks/av/include/private/media/AudioTrackShared.h)）：
...
**cblk_t头部核心字段**（[`AudioTrackShared.h`](frameworks/av/include/private/media/AudioTrackShared.h)）：
...
#### start()内部实现（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）
...
#### stop()内部实现（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）
...
#### pause()内部实现（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）
...
#### pauseAndWait() — 等待音量渐变完成（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）
...
#### flush()内部实现（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）
...
#### setVolume() — 即时音量设置（[`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp)）
...
| 问题 | 根因 | 源码定位 | 排查方法 |
|------|------|----------|----------|
| write()返回WOULD_BLOCK | 共享内存FIFO满 | [`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp) | 增大bufferSize或检查callback频率 |
| ERROR_DEAD_OBJECT | AudioFlinger重启 | [`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp) | 检查logcat中AudioFlinger crash |
| Underrun | App写入不够快 | [`Threads.cpp`](frameworks/av/services/audioflinger/Threads.cpp) | dumpsys audio查看underrun count |
| FAST flag被拒 | 传输模式不兼容 | [`AudioTrack.cpp`](frameworks/av/media/libaudioclient/AudioTrack.cpp) | 确认TRANSFER_CALLBACK/SYNC/OBTAIN |
| Offload不支持 | HAL不支持该格式 | [`AudioTrack.java`](frameworks/base/media/java/android/media/AudioTrack.java) | 检查audio_policy_configuration.xml |
...
- [`AudioRecord`](frameworks/base/media/java/android/media/AudioRecord.java) — 实现AudioRouting + MicrophoneDirection + AudioRecordingMonitor
...
**状态常量**（[`AudioRecord.java`](frameworks/base/media/java/android/media/AudioRecord.java)）：
...
#### native_setup参数传递（[`AudioRecord.java`](frameworks/base/media/java/android/media/AudioRecord.java)）
...
> - `CAMCORDER`标记`AUDIO_FLAG_CAPTURE_PRIVATE`（[`AudioRecord.cpp`](frameworks/av/media/libaudioclient/AudioRecord.cpp)）
...
#### start()（[`AudioRecord.cpp`](frameworks/av/media/libaudioclient/AudioRecord.cpp)）
...
#### stop()（[`AudioRecord.cpp`](frameworks/av/media/libaudioclient/AudioRecord.cpp)）
...
| 问题 | 根因 | 源码定位 | 排查方法 |
|------|------|----------|----------|
| read()返回WOULD_BLOCK | FIFO空 | [`AudioRecord.java`](frameworks/base/media/java/android/media/AudioRecord.java) | 检查RecordThread是否正常读HAL |
| ERROR_DEAD_OBJECT | AF重启 | [`AudioRecord.cpp`](frameworks/av/media/libaudioclient/AudioRecord.cpp) | 检查logcat AudioFlinger crash |
| FAILED_TRANSACTION | APM/AF竞态 | [`AudioRecord.cpp`](frameworks/av/media/libaudioclient/AudioRecord.cpp) | 自动重试3次 |
| FAST input被拒 | HAL不支持 | [`AudioFlinger.cpp`](frameworks/av/services/audioflinger/AudioFlinger.cpp) | 检查HAL FAST input声明 |
| 录音无声 | 路由到错误设备 | [`AudioFlinger.cpp`](frameworks/av/services/audioflinger/AudioFlinger.cpp) | dumpsys audio查看input路由 |
| 隐私指示器不消失 | stop()未调用 | [`AudioRecord.cpp`](frameworks/av/media/libaudioclient/AudioRecord.cpp) | 确保所有AudioRecord调用了stop() |
...
**源码位置**：
- Java层：[`MediaPlayer.java`](frameworks/base/media/java/android/media/MediaPlayer.java)
...
AudioOutput是MediaPlayerService内部对AudioSink的实现，关键代码在[`AudioOutput`](frameworks/av/media/libmediaplayerservice/MediaPlayerService.h)：
...
**源码位置**：[`SoundPool.java`](frameworks/base/media/java/android/media/SoundPool.java)
...
**源码位置**：
- Java层：[`MediaRecorder.java`](frameworks/base/media/java/android/media/MediaRecorder.java)
...
**Native层状态检查逻辑**（[`mediarecorder.cpp`](frameworks/av/media/libmedia/mediarecorder.cpp)）：
...
> **prepare()内部逻辑**（[`MediaRecorder.java`](frameworks/base/media/java/android/media/MediaRecorder.java)）：先将mPath/mFd/mFile转为FileDescriptor调用`_setOutputFile()`，再调用`_prepare()`触发Native侧的StagefrightRecorder初始化AudioSource+Encoder+Writer。
...
> **隐私敏感标记**（[`MediaRecorder.java`](frameworks/base/media/java/android/media/MediaRecorder.java)）：`VOICE_COMMUNICATION`和`CAMCORDER`默认标记为隐私敏感，禁止并发采集。可通过`setPrivacySensitive()`显式覆盖。
...
**源码位置**：
- Java层：[`MediaCodec.java`](frameworks/base/media/java/android/media/MediaCodec.java)
...
**源码位置**：
- Java层：[`MediaExtractor.java`](frameworks/base/media/java/android/media/MediaExtractor.java)
...
**源码位置**：[`MediaMetadataRetriever.java`](frameworks/base/media/java/android/media/MediaMetadataRetriever.java)
